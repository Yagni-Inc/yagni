...
public class MarketRepo {
    ...
    public void dayReport(DbConnection linkDb) {
        try {
            ...
            ResultSet result = state.executeQuery(
                    "SELECT product_id, sum(product_quanity) AS 'sum' FROM yagni_inv_db.purchase_history WHERE order_date > DATE_ADD(curdate(),INTERVAL -1 DAY) AND order_date <= curdate() GROUP BY product_id ORDER BY sum(product_quanity) DESC LIMIT 3;");
            result.next();
    ...
    public void weekReport(DbConnection linkDb) {
        ...
        try {
            ...
            /******** FOR WEEKLY REPORT ********/
            ResultSet result = state.executeQuery(
                    "SELECT product_id, sum(product_quanity) AS 'sum' FROM yagni_inv_db.purchase_history WHERE order_date > DATE_ADD(curdate(),INTERVAL -1 WEEK) AND order_date <= curdate() GROUP BY product_id ORDER BY sum(product_quanity) DESC LIMIT 3;");
            result.next();
            firstProduct = result.getString(1);         // 1st "Most popular order"
            result.next();
            secondProduct = result.getString(1);        // 2nd "most popular order"
            ...
            JOptionPane.showMessageDialog(null,
                    "<html>Most popular orders:<br>1. " + firstProduct + "<br>2. " + secondProduct + "<br>3. "
                            ...
                            + thirdAmt +
                            "</html>",
                    "Weekly Market Report", JOptionPane.INFORMATION_MESSAGE);
    ...
    public void monthReport(DbConnection linkDb) {
        ...
        try {
            ...
            /******** FOR MONTHLY REPORT ********/
            ResultSet result = state.executeQuery(
                    "SELECT product_id, sum(product_quanity) AS 'sum' FROM yagni_inv_db.purchase_history WHERE order_date > DATE_ADD(curdate(),INTERVAL -1 MONTH) AND order_date <= curdate() GROUP BY product_id ORDER BY sum(product_quanity) DESC LIMIT 3;");
            result.next();
            firstProduct = result.getString(1);         // 1st "Most popular order"
            result.next();
            ...
            JOptionPane.showMessageDialog(null,
                    "<html>Most popular orders:<br>1. " + firstProduct + "<br>2. " + secondProduct + "<br>3. "
                            ...
                            + thirdAmt +
                            "</html>",
                    "Monthly Market Report", JOptionPane.INFORMATION_MESSAGE);
